...
RELEASE_HASH="$(/usr/bin/shasum -a 256 "$RELEASE_BINARY" | /usr/bin/cut -d ' ' -f 1)"
if [[ ! -x "$APP/Contents/MacOS/telesram-rs" || ! -f "$STAMP" || "$(cat "$STAMP")" != "release:$RELEASE_HASH" ]]; then
  ...
  print -r -- "release:$RELEASE_HASH" > "$STAMP"
fi
